...
- `data/run-boundaries.json`: shared run boundary polygons, `{version, generated, boundaries:{"Garbage#Friday#208":[[lat,lng],...]}}`. Fetched at start by `lbApplySharedBoundaries()` (once per `version`, so **bump `version` when regenerating**). Generated from house locations by `lbRebuildAllBoundaries()` (a "nearest house" tessellation, `lbTessellate`) and exported from the app's Boundaries settings.
