"""Garbage's route-map PDFs have unreadable title text (a font export bug turns every letter into an
unrelated Unicode codepoint - digits included), so which run each page shows can't be read off the page.
Instead, identify it by geometry: try the page's route against every Garbage run that runs on that page's
day, and keep whichever one it lines up with clearly best. The day itself comes from the PDF's filename
(each file is one day, e.g. "1_202604_Garbage_Monday.pdf"), which is plain text and not affected.
...
def run(only_day=None):
    ...
    for day, pdf in pdfs.items():
        ...
        for page in range(1, n + 1):
            ...
            # cheap first pass: rank every candidate by coarse (normalised) correlation
            coarse = []
            ...
            # expensive second pass: precisely fit only the top few, and decide by that - the coarse score
            # alone doesn't discriminate reliably enough on its own to identify the run.
            precise = []
            ...
            rec = dict(tag=tag, houses=len(best_H), fit_m=round(float(f_cost), 1), scale_m_per_pt=round(float(p[0]), 2), new=gn, old=go)
            ...
            print(f"  p{page}: run {best_run} (fit {f_cost:5.1f}m vs runner-up {runner_up:5.1f}m)  ({len(best_H)} houses) {gn['km'] if gn else 0:.1f}km | NEW {fmt(gn)} | OLD {fmt(go)}")
    ...
    print(f'\n{len(ok)} of {len(results)} pages identified and placed; wrote work/compare-results-GAR.json, work/routes-m-GAR.json')
